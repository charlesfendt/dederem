 ...
 * SEE https://gist.github.com/jsfeng/2629858#file-fileservlet-java
 *
 ...
 *
 * @author BalusC, modified by charles
...
public class DownloadServlet extends HttpServlet {
	...
	private static final long serialVersionUID = -8017782259477498605L;
	...
	/** Singleton for DEDEREM configuration. */
	@Inject
	 ...
	 */
	private void processRequest(final HttpServletRequest request, final HttpServletResponse response, final boolean content) throws IOException {
		...
		final File file = new File(this.configService.getRepoDir(), URLDecoder.decode(requestedFile, "UTF-8"));
		...
		if (ifNoneMatch != null && DownloadServlet.matches(ifNoneMatch, eTag)) {
		...
		if (ifMatch != null && !DownloadServlet.matches(ifMatch, eTag)) {
		...
		if (range != null) {
			...
			if (ranges.isEmpty()) {
				for (final String part : range.substring(6).split(",")) {
					...
					long start = DownloadServlet.sublong(part, 0, part.indexOf("-"));
					long end = DownloadServlet.sublong(part, part.indexOf("-") + 1, part.length());
		...
		if (contentType.startsWith("text")) {
			...
			acceptsGzip = acceptEncoding != null && DownloadServlet.accepts(acceptEncoding, "gzip");
		...
		// the browser, then set to inline, else attachment which will pop a 'save as' dialogue.
			...
			disposition = accept != null && DownloadServlet.accepts(accept, contentType) ? "inline" : "attachment";
		}
		
		// Initialize response.
		response.reset();
		response.setBufferSize(DownloadServlet.DEFAULT_BUFFER_SIZE);
		response.setHeader("Content-Disposition", disposition + ";filename=\"" + fileName + "\"");
		response.setHeader("Accept-Ranges", "bytes");
		response.setHeader("ETag", eTag);
		response.setDateHeader("Last-Modified", lastModified);
		response.setDateHeader("Expires", System.currentTimeMillis() + DownloadServlet.DEFAULT_EXPIRE_TIME);
		...
		try {
			...
			if (ranges.isEmpty() || ranges.get(0) == full) {
				...
				if (content) {
					if (acceptsGzip) {
						...
						output = new GZIPOutputStream(output, DownloadServlet.DEFAULT_BUFFER_SIZE);
					} else {
					...
					// Copy full range.
					DownloadServlet.copy(input, output, r.start, r.length);
				}
				...
				response.setContentType("multipart/byteranges; boundary=" + DownloadServlet.MULTIPART_BOUNDARY);
				response.setStatus(HttpServletResponse.SC_PARTIAL_CONTENT); // 206.
				...
				if (content) {
					...
					for (final Range r : ranges) {
						...
						sos.println();
						sos.println("--" + DownloadServlet.MULTIPART_BOUNDARY);
			...
			DownloadServlet.close(output);
			DownloadServlet.close(input);
		}
	}
	 ...
	 *             If something fails at I/O level.
	 */
	private static void copy(final RandomAccessFile input, final OutputStream output, final long start, final long length) throws IOException {
		final byte[] buffer = new byte[DownloadServlet.DEFAULT_BUFFER_SIZE];
		int read;
